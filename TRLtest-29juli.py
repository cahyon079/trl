...
def main():
    ...
    st.title("📊 Aplikasi Penilaian Technology Readiness Level (TRL)")
    st.write("Alat untuk menilai tingkat kematangan teknologi organisasi Anda")
    ...
    with st.sidebar:
        ...
        page = st.radio("Pilih halaman:", page_options)
    ...
    if max_score > 0:
        ...
        if progress >= MIN_SCORE_THRESHOLD and level < 9:
            ...
            if st.button(f"Lanjutkan ke TRL {level+1}", key=f"next_{level}", 
                ...
                st.session_state['active_page'] = f"TRL {level+1}: {get_trl_data(level+1)['title']}"
                st.rerun()
